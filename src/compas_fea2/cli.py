...
def one_o_one() -> None:
    ...
    click.echo("\n🔧 compas_fea2 CLI - Finite Element Analysis Made Easy")
    click.echo("=" * 55)
    click.echo("\nAvailable Commands:")
...
def init_backend(backend: str, clean: bool) -> None:
    ...
    try:
        if 'init_plugin' not in globals():
            click.echo("❌ Error: fea2_extension module not available. Cannot initialize backend.", err=True)
            sys.exit(1)
        
        init_plugin(HOME, backend, clean)
        click.echo(f"✅ Successfully initialized backend: {backend}")
...
def change_setting(backend: str, setting: str, value: str) -> None:
    ...
    try:
        ...
        click.echo(f"❌ Error: Backend '{module_name}' not found or not installed", err=True)
        sys.exit(1)
    ...
    try:
        ...
        if not os.path.exists(env_path):
            click.echo(f"⚠️  Warning: .env file not found at {env_path}. Creating new file.")
            with open(env_path, 'w') as f:
                f.write(f"# Environment file for {module_name}\n")
        
        dotenv.set_key(env_path, setting.upper(), value)
        click.echo(f"✅ {setting.upper()} set to {value} for {module_name}")
        
    except AttributeError:
        click.echo(f"❌ Error: Backend module '{module_name}' does not have a HOME attribute", err=True)
        sys.exit(1)
    except PermissionError:
        click.echo(f"❌ Error: Permission denied when writing to {env_path}", err=True)
        sys.exit(1)
    except Exception as e:
        click.echo(f"❌ Error changing setting: {e}", err=True)
        sys.exit(1)
...
def list_backends() -> None:
    ...
    click.echo("🔍 Searching for installed compas_fea2 backends...")
    ...
    if backends:
        ...
        click.echo("\n❌ No compas_fea2 backends found.")
        click.echo("📦 Install a backend with: pip install compas_fea2_<backend_name>")
...
def backend_info(backend: str) -> None:
    ...
    try:
        m = importlib.import_module(module_name)
        click.echo(f"📋 Backend Information")
        click.echo("=" * 30)
        ...
        click.echo(f"❌ Backend '{backend}' not installed")
        sys.exit(1)
...
def show_settings(backend: str) -> None:
    ...
    try:
        ...
        if not os.path.exists(env_path):
            click.echo(f"❌ No settings file found for {module_name}")
            return
        
        click.echo(f"⚙️  Settings for {module_name}:")
        click.echo(f"📁 File: {env_path}")
        click.echo("-" * 40)
...
def create(name: str, backend: str, template: str) -> None:
    ...
    templates = {
        ...
        'truss': _create_truss_template,
        'frame': _create_frame_template,
        'shell': _create_shell_template
    }
    
    templates[template](project_dir, backend)
    click.echo(f"✅ Project '{name}' created with {template} template")
...
def validate_project() -> None:
    ...
    if missing:
        click.echo(f"⚠️  Missing directories: {', '.join(missing)}")
    else:
...
def validate(model_file: str, output: Optional[str]) -> None:
    ...
    if not os.path.exists(model_file):
        click.echo(f"❌ Model file '{model_file}' not found")
        sys.exit(1)
    ...
    try:
        ...
        click.echo(f"❌ Error validating model: {e}")
        sys.exit(1)
...
def info(model_file: str) -> None:
    ...
    try:
        ...
        click.echo(f"📊 Model: {model.name or 'Unnamed'}")
        click.echo("=" * 30)
        ...
        click.echo(f"❌ Error reading model: {e}")
        sys.exit(1)
...
def export(results_file: str, format: str, output: Optional[str]) -> None:
    ...
    if not os.path.exists(results_file):
        click.echo(f"❌ Results file '{results_file}' not found")
        sys.exit(1)
...
def show() -> None:
    ...
    try:
        from compas_fea2 import VERBOSE, POINT_OVERLAP, GLOBAL_TOLERANCE, PRECISION
        
        click.echo("⚙️  Global Configuration:")
        click.echo("=" * 25)
        ...
        click.echo("❌ Configuration variables not available")
...
def set(key: str, value: str) -> None:
    ...
    if key.upper() not in valid_keys:
        click.echo(f"❌ Invalid key. Valid keys: {', '.join(valid_keys)}")
        sys.exit(1)
    
    # Update .env file
    env_path = os.path.join(HOME, '.env')
    dotenv.set_key(env_path, key.upper(), value)
    click.echo(f"✅ {key.upper()} set to {value}")
...
def reset() -> None:
    ...
    if click.confirm("Reset all configuration to defaults?"):
        try:
            ...
            click.echo("❌ Reset function not available")
...
def convert(input_file: str, output_file: str, backend_from: str, backend_to: str) -> None:
    ...
    click.echo(f"🔄 Converting from {backend_from} to {backend_to}...")
...
def visualize(model_file: str, scale: float) -> None:
    ...
    try:
        ...
        click.echo("🖼️  Launching model viewer...")
...
def doctor(check_backends: bool, check_dependencies: bool) -> None:
    ...
    click.echo("🔍 Running compas_fea2 diagnostics...")
    ...
    if check_backends or not (check_backends or check_dependencies):
        ...
        for backend in common_backends:
            try:
                ...
                click.echo(f"  ✅ compas_fea2_{backend}: Installed")
            except ImportError:
                click.echo(f"  ❌ compas_fea2_{backend}: Not installed")
...
def examples() -> None:
    ...
    if 1 <= choice <= len(examples):
        ...
        click.echo(f"✅ Downloaded {examples[choice-1]} example")
    else:
...
def version() -> None:
    ...
    try:
        ...
        click.echo(f"📦 compas_fea2 version: {__version__}")
    except ImportError:
    ...
    for backend in ['opensees', 'abaqus', 'ansys', 'sofistik', 'diana']:
        try:
            ...
            click.echo(f"  📌 compas_fea2_{backend}: {version}")
        except ImportError:
            pass
