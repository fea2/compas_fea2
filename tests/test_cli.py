...
from compas_fea2.cli import (
    main, one_o_one, init_backend, change_setting, list_backends, 
    backend_info, show_settings, project, model, results, config, 
    tools, docs, examples, version, validate_backend_name, validate_setting_name
)
...
class TestToolsCommands(unittest.TestCase):
    ...
    def test_tools_convert(self):
        ...
        result = self.runner.invoke(tools, [
            "convert", "input.json", "output.json", 
            "--backend-from", "opensees", "--backend-to", "abaqus"
        ])
        self.assertEqual(result.exit_code, 0)
