...
The `Problem` class in `compas_fea2` is a central component for defining and managing finite element analysis problems.
It encapsulates all the necessary information required to set up and solve an FEA problem, including the model, analysis type, boundary conditions, loads, and solver settings.
...
problem = Problem(model=model, analysis_type='static')
